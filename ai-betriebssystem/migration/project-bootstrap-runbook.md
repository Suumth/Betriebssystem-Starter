...
Der Ablauf ist tool-unabhaengig: ChatGPT, Claude, Codex, Gemini und spaetere
KI-Tools starten alle ueber dieselbe `PROJECT.md` im Repo-Root.
...
## 2. Eingaben fuer ein neues Projekt
...
| Default-Branch |  |
| Projekttyp |  |
| lokaler Checkout-Pfad fuer lokale Agenten |  |
| AI-Vault-Projektordner |  |
| AI-Vault-Online-Einstieg fuer Cloud-KIs |  |
| Produktvision |  |
| Zielnutzer |  |
| Nicht-Ziele |  |
| Protected Areas |  |
...
5. `README.md` anlegen oder pruefen:
   - Menschlicher Einstieg
   - kurzer Produktzweck
   - Verweis auf `PROJECT.md` fuer KI-Arbeit
...
8. Issue-Template aus `templates/github_issue_task.md` uebernehmen.
9. PR-Template aus `templates/pull-request-template.md` uebernehmen oder den PR-Body-Standard im Repo dokumentieren.
10. Labels aus `templates/labels.yml` per Label-Sync oder manuell in GitHub anlegen.
11. Tool-Projektanweisungen auf den duennen Pointer aus `templates/chatgpt-project-instructions.md` reduzieren.
12. Ticket 0 aus `templates/ticket-0-bootstrap.md` erstellen und erst nach vollstaendigem Contract mit `agent:ready` markieren.
...
Die Dateinamen duerfen projektspezifisch abweichen, wenn `PROJECT.md` die
...
- `needs-human` ist kein Gruen-Signal.
...
- welche Bootstrap-Dateien angelegt oder geprueft wurden
...
Mindestvalidierung fuer den Bootstrap-PR:

```bash
git diff --check
```

Zusaetzlich ausfuehren, falls vorhanden:

- Markdownlint oder andere Markdown-Checks
- YAML-Parse fuer Label-Dateien
- Build/Test/Lint des Zielprojekts
- Preview-/Screenshot-Smoke fuer UI-Projekte
...
Der Bootstrap-PR ist die Primary Closeout Source und enthaelt:
...
- `PROJECT.md` im Repo-Root existiert und ueber GitHub erreichbar ist
- jedes KI-Tool denselben Einstiegspunkt bekommt
- `PROJECT.md` auf AI-Vault-Quellen verweist, ohne sie vollstaendig zu duplizieren
...
- nach gruenem Merge `main` synchronisiert und sauber ist

## 9. Fertiger Startprompt

Nutze diesen Prompt fuer ChatGPT, Claude, Codex, Gemini oder ein anderes KI-Tool,
wenn ein neues Projekt vorbereitet werden soll:

```text
Arbeite als Bootstrap-Agent fuer ein neues AI-Betriebssystem-2.0-Projekt.

Ziel:
Bereite aus der folgenden Produktidee ein bootstrap-fertiges Projekt-Setup vor.
Das Ergebnis muss PROJECT.md-first sein: Jede KI liest spaeter zuerst
...
- Projekttyp:
- Default-Branch:
- lokaler Checkout-Pfad fuer lokale Agenten, falls bekannt:
- AI-Vault-Projektordner oder gewuenschter Ordner:
- AI-Vault-Online-Einstieg fuer Cloud-KIs, falls vorhanden:
...
2. Erzeuge oder plane diese Dateien:
   ...
   - .github/pull_request_template.md
   - .github/labels.yml
   - AI-Vault-Dateien fuer Vision, Zielbild, Produktbrief, Roadmap, Entscheidungen, Risiken und Lessons Learned
3. PROJECT.md ist der zentrale Einstiegspunkt fuer ChatGPT, Claude, Codex, Gemini und spaetere Tools.
4. Tool-spezifische Projektanweisungen duerfen nur auf die GitHub-URL von PROJECT.md zeigen.
