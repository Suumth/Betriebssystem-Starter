...
Ein neutraler Starter fuer Teams, die AI-Agentenarbeit ueber GitHub, klare Tickets, Evidence und verbindliche Human Gates steuern wollen.
...
4. Fuehre genau diesen Setup-Befehl aus:

```bash
bash scripts/setup.sh
```

Danach fuehrt dich `START_HERE.md` durch das erste Projekt und Ticket 0.

## Was ist das?

Der Starter ist eine oeffentliche Erstinstallation fuer ein AI-Betriebssystem mit AI Vault. Er bringt Methode, Contracts, Templates, Prompts, Demo-Projekt, Demo-Vault und Readiness-Checks mit.

Die operative Logik bleibt einfach:

- GitHub ist die operative Wahrheit fuer Issues, PRs, Labels, Reviews und Evidence.
- Der AI Vault ist Projektgedaechtnis und Strategie, nicht der operative Tasklog.
...
- Fuer Produkt- und Engineering-Teams, die Agentenarbeit reproduzierbar machen wollen.
- Fuer Einzelpersonen, die Projekte mit klaren Tickets, Reviews und Evidence fuehren wollen.
- Fuer Organisationen, die AI-Tooling nutzen wollen, ohne operative Wahrheit aus GitHub herauszuziehen.
...
- `ai-vault/` ist eine leere, sinnvolle Vault-Erstinstallation fuer Obsidian oder einen Markdown-Editor.
...
- `docs/first-installation.md` und `docs/first-project.md` erklaeren die Installation und das erste Projekt im Detail.
- `public-readiness/` dokumentiert Inventar, Sanitization und Release-Checks.
- `scripts/public-readiness-check.sh` prueft private Treffer, sensible Treffer, Pflichtdateien, Pflichtlabels und lokale Altlasten.

## Nach dem Setup

1. Lies `START_HERE.md`.
2. Oeffne `ai-vault/` in Obsidian oder einem Markdown-Editor.
3. Nutze `examples/demo-project/` als Vorlage fuer dein erstes echtes Projekt.
...
6. Fuehre vor jeder Weitergabe `bash scripts/public-readiness-check.sh` aus.

## Wichtige Grenze

Der Starter enthaelt keine privaten Projekte, keine echten Roadmaps, keine echten Issue- oder PR-Historien, keine lokalen Pfade und keine Secrets.
